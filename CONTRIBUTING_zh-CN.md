...
- **定义指标**：包含具体、可量化的成功标准
- **校对检查**：检查错别字、格式、清晰度
- **检查它在每个工具中都能完整保留**：运行 `./scripts/test-convert-outputs.sh`。它会重新生成每个工具的输出，并确认你的智能体能通过每个转换器。添加或编辑智能体会改变生成产物，因此会报告 **manifest drift**；这是预期行为。查看变更后用 `--update` 再跑一次，并将刷新后的 `scripts/convert-outputs.sha256` 与智能体一并提交。CI 会运行同样的检查。

这些检查之所以存在，是因为每天有数千人在十几种不同工具中依赖这些智能体。转换器里的一次小疏忽，或某个文件里多出来的一个引号，会悄无声息地同时影响到所有人。在本地运行测试大约只需一分钟，却能让你的工作以原样到达每一位使用者。
